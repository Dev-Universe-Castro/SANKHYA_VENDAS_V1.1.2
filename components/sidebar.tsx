...
import { Home, Users, ChevronLeft, ChevronRight, LogOut, UserCircle, LayoutGrid, Package, ShoppingCart, Calendar, DollarSign, Settings, Download, Loader2, Cloud, CloudOff } from "lucide-react"
    ...
    <>
      ...
      >
        ...
        <div className="border-b border-sidebar-border shrink-0">
          ...
          {/* Status e Carga Offline - Mobile apenas, logo abaixo da logo */}
          <div className="lg:hidden px-4 pb-4 space-y-2">
            {/* Status Online/Offline */}
            <div className={cn(
              "flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium justify-center",
              isOnline 
            ...
            {/* Botão de Carga Offline */}
            <Button
              onClick={realizarCargaOffline}
              disabled={!isOnline || isLoading}
              variant="ghost"
              size="sm"
              className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-sidebar-foreground/80 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground h-9"
            >
              {isLoading ? (
                <>
        ...
        <div className="flex-1 p-4 flex flex-col overflow-y-auto min-h-0 scrollbar-hide">
          {!isCollapsed && (
            <p className="text-xs uppercase tracking-wider text-sidebar-foreground/60 mb-3 px-3 flex-shrink-0">Expedição</p>
          )}
